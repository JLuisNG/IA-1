...
fetch('../json/agencies.json')
  ...
  .then(data => {
    agencies = data;
    localStorage.setItem('agencies', JSON.stringify(agencies)); // Guardar en localStorage
    loadAgencies();
  })
  .catch(error => console.error('Error loading agencies:', error));

function loadAgencies() {
  const storedAgencies = JSON.parse(localStorage.getItem('agencies')) || agencies;
  const tbody = document.querySelector('#agenciesTable tbody');
  tbody.innerHTML = '';
  storedAgencies.forEach(agency => {
    tbody.innerHTML += `
      <tr>
        <td>${agency.nombre}</td>
        <td>${agency.referidos}</td>
        <td>${agency.pacientesActivos}</td>
